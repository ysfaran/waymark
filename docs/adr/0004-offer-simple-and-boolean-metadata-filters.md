...
Waymark will provide simple accumulated `--scopes`, `--kinds`, `--tags`, and
...
arbitrary grouping. The Boolean grammar supports `scope:`, `kind:`, and `tag:`
predicates. This preserves short agent commands while avoiding the expressiveness
limits of specialized flags; the two syntaxes are kept separate so their
composition is never implicit. The supporting CLI precedent is captured in
